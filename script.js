...
const WHATSAPP_NUMBER = "918574481775";
...
async function syncToGitHub() {
    ...
    const response = await fetch(url, {
        ...
        body: JSON.stringify({
            ...
            content: btoa(unescape(encodeURIComponent(JSON.stringify(currentData)))),
            sha: sha
        })
    });
...
function renderAll() {
    ...
    currentData.catalog.forEach(item => {
        const card = `<div class="product-item"><img src="${item.link}"><div style="padding:8px;"><strong>${item.name}</strong><br><a href="https://wa.me/${WHATSAPP_NUMBER}?text=Details: ${item.name}" class="wa-btn" style="color: green; font-weight: bold; text-decoration: none;">WhatsApp Inquiry</a></div></div>`;
